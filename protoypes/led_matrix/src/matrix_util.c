...
void animateGo(unsigned char*bitmap, int bitmap_size) {
    ...
    drawPixels(g_pixel_list, sizeof(g_pixel_list) / sizeof (unsigned char), bitmap);
    drawPixels(o_pixel_list, sizeof(o_pixel_list) / sizeof (unsigned char), bitmap);
    drawPixels(bang_pixel_list, sizeof(bang_pixel_list) / sizeof (unsigned char), bitmap);
    while (1) {
        HUB75E_setDisplayBuffer(bitmap);
         HUB75E_displayBufferPixels(bitmap);
    }
